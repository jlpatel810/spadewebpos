...
class Sale extends CI_Model
{
	...
	public function get_payment_options($giftcard = TRUE, $reward_points = FALSE)
	{
		...
		}					$payments[$this->lang->line('sales_payment_stamps')] = $this->lang->line('sales_payment_stamps');	 
	...
	public function get_all_suspended($customer_id = NULL)
	{
		if($customer_id == -1)
		{
			$query = $this->db->query('select sale_id, sale_id as suspended_sale_id, sale_status, sale_time, dinner_table_id, customer_id, comment from '
				...
				. $this->db->dbprefix('sales_suspended').' order by sale_id desc limit 0,5  ');								
		}
		else
		{
			$query = $this->db->query('select sale_id, sale_id as suspended_sale_id, sale_status, sale_time, dinner_table_id, customer_id, comment from '
				...
				. ' union select sale_id, sale_id*-1 as suspended_sale_id, 2 as sale_status, sale_time, dinner_table_id, customer_id, comment from '
				. $this->db->dbprefix('sales_suspended') . ' where customer_id = ' . $customer_id.' order by sale_id desc limit 0,5 ');								 
		}
	...
	public function get_item_info($id=0, $qty=1000000000)
	{
		 $qry="SELECT * FROM `ospos_items_discont_price` WHERE item_id=$id and `qty`<=$qty and Isactive=1  order by qty desc limit 0,1";
