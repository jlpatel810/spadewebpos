...
class Sale_lib
{
	...
	public function __construct()
	{
		...
		$this->CI->load->model('Rule');
		
	}

	public function get_deals($item_id)
	{
	    return $items_deals=$this->CI->Rule->loadselecteddata($item_id);
	}
	
	public function get_totalDiscount($rule_id)
	{
	    return $rule_details=$this->CI->Rule->get_info($rule_id);
	}
...
/*		    foreach ($deals as $deal) {
		    ...
		    $withDiscount = $this->get_item_total($discountedItems,$item['price'],$discount,TRUE);
		    $withoutDiscount = $this->get_item_total($current_quantity,$item['price'],0,FALSE);
		    
		    
		    $discount_amount = 0;
		    $discount_amount = $this->get_item_discount($discountedItems, $item['price'], $item['discount']);;*/

		        $discount_amount = $this->get_item_discount($item['quantity'], $item['price'], $item['discount']);
		        $discount_amount = bcadd($discount_amount , $item['discounted_total']);
		    }
	        else 
	        {
	            $discount_amount = $this->get_item_discount($item['quantity'], $item['price'], $item['discount']); 
	...
	public function edit_item($line, $description, $serialnumber, $quantity, $discount, $price,$item_id_new=0)
	{
		$items = $this->get_cart();
		if(isset($items[$line]))
		{
				/*if($item_id_new){
						...
						$dataArray = $this->CI->sale->get_item_info($item_id_new, $quantity);					

						if($dataArray[0]->price!=''){					

								$pricenew=$dataArray[0]->price;
								
								//$discount= ($price*$quantity)-($pricenew*$quantity); 
						//$items[$updatekey]['discount']=$discount;
						}
				} */
	...
	public function apply_deals_Discount($items)
	{
	    $this->set_cart($items);
	}
